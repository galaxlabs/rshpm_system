...
def _otp_store(phone: str, otp: str):
	_cache().set_value(_otp_key(phone), otp, expires_in_sec=OTP_TTL_SECONDS)
	_cache().set_value(_otp_key(phone) + ":ts", str(now_datetime()), expires_in_sec=OTP_TTL_SECONDS)


def _otp_get(phone: str):
	return _cache().get_value(_otp_key(phone))


def _otp_clear(phone: str):
	_cache().delete_value(_otp_key(phone))
	_cache().delete_value(_otp_key(phone) + ":ts")


def _send_otp_stub(phone: str, otp: str):
	# DEV ONLY: logs OTP to error log
	frappe.log_error(title="PORTAL_OTP", message=f"phone={phone} otp={otp}")
...
def _current_client_or_throw():
	...
	client = frappe.db.get_value("Client", {"user": frappe.session.user}, "name")
	if not client:
		frappe.throw("No Client linked to this user.", title="Client Not Linked")
	return client
...
def otp_login_request(phone: str, cnic: str = None):
	"""
	Request OTP for login. (Minimal version)
	"""
	phone = (phone or "").strip()
	if not phone:
		frappe.throw("Phone is required.")

	# ensure client exists by phone_normalized (adjust if your field differs)
	client = frappe.db.get_value("Client", {"phone_normalized": phone}, "name")
	if not client:
		frappe.throw("No client found for this phone.", title="Not Registered")

	otp = _otp_generate()
	_otp_store(phone, otp)
	_send_otp_stub(phone, otp)

	return {"ok": True, "message": "OTP sent.", "ttl_seconds": OTP_TTL_SECONDS}


@frappe.whitelist(allow_guest=True)
def otp_login_verify(phone: str, otp: str, cnic: str = None):
	"""
	Verify OTP and create a session (sets sid cookie).
	"""
	phone = (phone or "").strip()
	otp = (otp or "").strip()
	if not phone or not otp:
		frappe.throw("Phone and OTP are required.")

	stored = _otp_get(phone)
	if not stored:
		frappe.throw("OTP expired. Please request again.")
	if otp != str(stored).strip():
		frappe.throw("Invalid OTP.")

	_otp_clear(phone)

	# find client by phone_normalized (adjust if needed)
	client = frappe.db.get_value("Client", {"phone_normalized": phone}, "name")
	if not client:
		frappe.throw("Client not found.", title="Not Registered")

	# login as the linked user (Client.user)
	user_id = frappe.db.get_value("Client", client, "user")
	if not user_id:
		frappe.throw("Client has no linked user.", title="Client User Missing")
	...
	return {"ok": True, "user": user_id, "client": client, "sid": frappe.session.sid}


@frappe.whitelist()
def my_profile():
	client_name = _current_client_or_throw()
...
# 	if not frappe.db.has_column("Client", "user"):
